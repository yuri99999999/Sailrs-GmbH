...
type CalendarViewProps = CalendarState & CalendarActions &
        { taskBoardState: TaskBoardState } & RouteComponentProps<{id: string}> & {
};
        ...
        <>
            ...
            {open ?
                <TaskDialog
                    open={true}
                    record={props.record}
                    teamOrStories={props.taskBoardState.activeBoard.teamOrStories}
                    taskStatuses={props.taskBoardState.activeBoard.taskStatuses}
                    board={props.board}
...
const CalendarView: React.FC<CalendarViewProps> = (props) => {
    ...
    if (props.match.params.id) {
        if (props.taskBoardState.activeBoard._id !== props.match.params.id) {
            const index = props.taskBoardState.boards.findIndex(x => x._id === props.match.params.id);
    ...
    const stickys = props.taskBoardState.activeBoard.records.filter(x => {
        if (!x.dueDate) {
            return false;
        }
    ...
    return (
        <div className={clsx(classes.root)}>
            <style dangerouslySetInnerHTML={{__html: props.taskBoardState.activeBoard.calendarStyle}}></style>
            <div className={clsx(classes.calendarTitle)}>
                <div className={clsx(classes.calendarTitleToolBar)}>
                    ...
                    <Typography variant="h6" display="inline" style={{marginLeft: '3em', cursor: 'pointer'}}
                        ...
                        props.taskBoardState.activeBoard.name}</Typography>
                </div>
            </div>
                                        ...
                                        <div style={{position: 'relative'}}>
                                            ...
                                            <div style={{paddingTop: '20px', minHeight: '80px'}}>
                                                {stickys.filter(x => {
                                                    ...
                                                    <CalendarItem
                                                        board={props.taskBoardState.activeBoard}
                                                        key={x._id}
                                                        record={x} />
                                                )}
                                            </div>
